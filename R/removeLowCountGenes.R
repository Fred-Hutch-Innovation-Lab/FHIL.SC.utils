...
#'   objs <- lapply(objs, removeLowCountGenes, min.cells = 100)
#'   ## apply filter
#'   objs <- lapply(objs, removeLowCountGenes, min.cells = 100, calculate_only = FALSE)
#' }
removeLowCountGenes <- function(obj,
                           min.cells,
                           assay = 'RNA',
                           calculate_only = TRUE){
